...
현재 서비스에서 사용할 **최종 진입점**은 `pipeline/preprocessing_pipeline.py`입니다.

```python
from img_preprocessing.pipeline import preprocess_for_service
```

이 함수가 `raw image → hybrid crop → view enhancement → LLM-ready image` 흐름을 한 번에 실행합니다.

`crop/hybrid_preprocessor.py`는 이 파이프라인 안에서 사용하는 **크롭 단계의 핵심 모듈**입니다.
...
서비스/API에서는 `img_preprocessing.pipeline.preprocessing_pipeline`을 호출하면 됩니다.  
크롭만 따로 검증하고 싶을 때는 `img_preprocessing.crop.hybrid_preprocessor`를 실행하면 됩니다.  
`crop/board_cropper.py`와 `crop/yolo_world_detector.py`는 하이브리드 내부에서 사용하는 핵심 모듈입니다.
...
├─ enhance/
│  ├─ crop_enhancer.py       # crop 후처리 orchestration
│  ├─ image_ops.py           # 공통 이미지 유틸
│  ├─ metrics.py             # view 품질/보정 metric
│  ├─ profile_detector.py    # 흰 배경/검은 배경 프로필 선택
│  ├─ profiles/
│  │  ├─ scan_document.py    # 흰 배경 문서/화이트보드 보정
│  │  └─ dark_board.py       # 칠판/검은 배경 보정
│  └─ crop_enhancement_experiment.py # 이미 crop된 이미지 후처리 실험 CLI
├─ pipeline/
│  ├─ preprocessing_pipeline.py   # 서비스용 raw→crop→enhance 진입점
│  └─ preprocessing_experiment.py # 서비스 파이프라인 CLI wrapper
└─ README.md
```
...
.venv/bin/python -m img_preprocessing.pipeline.preprocessing_experiment \
  --input "sample_images/notes/mock-presentation/mock-network-board.jpg" \
  --output-dir "outputs/service_preprocess_demo" \
  --debug
```

YOLO 없이 OpenCV 후보만 쓰려면 `--no-yolo`를 사용합니다.
...
### `enhance/crop_enhancer.py`

이미 crop된 이미지를 LLM에 전달할 `view` 이미지로 후처리하는 진입점입니다.

담당 역할:

1. crop 이미지 로드
2. `scan_document` / `dark_board` 프로필 선택
3. 프로필별 view 보정 실행
4. 최종 `*_view.jpg`와 `*_summary.json` 저장

세부 구현은 다음 모듈로 분리되어 있습니다.

```text
profile_detector.py
metrics.py
profiles/scan_document.py
profiles/dark_board.py
image_ops.py
```
...
현재 YOLO 단독 CLI는 별도 파일로 유지하지 않습니다.

YOLO 후보를 확인하려면 하이브리드 실행 시 `--debug-dir`를 지정해서 후보 이미지와 summary를 확인합니다.
...
10. crop 후처리 view 생성
11. 서비스용 raw→crop→enhance 파이프라인

## 주의사항

1. 서비스/API에서는 `pipeline/preprocessing_pipeline.py`가 최종 전처리 진입점입니다.
...
4. `HEIC`처럼 OpenCV가 바로 읽지 못하는 포맷은 실패할 수 있습니다.
5. 보드 경계가 거의 보이지 않거나 조명이 심하게 반사된 이미지는 오검출될 수 있습니다.
6. YOLO-World는 사전학습 open-vocabulary 모델이므로 모든 강의실 상황에서 항상 정답을 보장하지 않습니다.

## 요약

최종 사용은 `pipeline/preprocessing_pipeline.py`입니다.
...
`enhance/crop_enhancer.py`는 crop 결과를 `view` 이미지로 보정하고, 서비스 파이프라인은 이 둘을 묶어 LLM-ready 이미지를 생성합니다.
